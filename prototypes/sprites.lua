...
--- @alias khaoslib_sprites.Icon data.IconData
--- @alias khaoslib_sprites.Icons data.IconData[]
--- @alias khaoslib_sprites.IconStructures data.AchievementPrototype|data.AmmoCategory|data.AsteroidChunkPrototype|data.BaseModifier|data.EntityPrototype|data.FluidPrototype|data.ItemGroup|data.ItemPrototype|data.QualityPrototype|data.RecipePrototype|data.ScriptedTechnologyTrigger|data.ShortcutPrototype|data.SpaceConnectionPrototype|data.SpaceLocationPrototype|data.SurfacePrototype|data.TechnologyPrototype|data.TilePrototype|data.TipsAndTricksItem|data.VirtualSignalPrototype
--- @alias khaoslib_sprites.IconAll khaoslib_sprites.Icon|khaoslib_sprites.Icons|khaoslib_sprites.IconStructures

--- Traverses a sprite or animation or icons or a table of them and applies the given function to each sprite/animation/icons found.
--- @generic T : khaoslib_sprites.AnimationAll|khaoslib_sprites.SpriteAll|khaoslib_sprites.IconAll
--- @param sprites T The sprite or animation or an object containing icons or a table of them.
--- @param fn fun(sprite: khaoslib_sprites.Animation|khaoslib_sprites.Sprite|khaoslib_sprites.Icon): nil A function that takes a sprite or animation or icons copy and applies modifications to it.
--- @return T copy A copy of the given animation or sprite or icons (object) or a table of them with the modifications applied.
--- @nodiscard
function khaoslib_sprites.traverse(sprites, fn)
  ...
  --- @cast sprites khaoslib_sprites.AnimationAll|khaoslib_sprites.SpriteAll|khaoslib_sprites.IconAll

  --- @type khaoslib_sprites.AnimationAll|khaoslib_sprites.SpriteAll|khaoslib_sprites.IconAll
  local copy = {}
  ...
  elseif sprites.icons then
    copy.icons = khaoslib_sprites.traverse(sprites.icons, fn)
  elseif sprites.icon then
    copy = util.table.deepcopy(sprites)
    fn(copy --[[@as khaoslib_sprites.Icon]])
  else
...
--- Replaces the given animations and sprites filenames and icons.
--- @generic T : khaoslib_sprites.AnimationAll|khaoslib_sprites.SpriteAll|khaoslib_sprites.IconAll
--- @param sprites T The animation or sprite or icons (object) or a table of them.
--- @param replacements {[string]: string} A table where the key is the original filename and the value is the new filename.
--- @return T copy A copy of the given animation or sprite or icons (object) or a table of them with the replaced filenames.
--- @nodiscard
function khaoslib_sprites.replace(sprites, replacements)
  ...
  return khaoslib_sprites.traverse(sprites, function(sprite)
    if sprite.icon and replacements[sprite.icon] then
      sprite.icon = replacements[sprite.icon]
    elseif sprite.filename and replacements[sprite.filename] then
      sprite.filename = replacements[sprite.filename]
    end
  end)
end

--- Tints animations and sprites with possible layers and icons.
--- @generic T : khaoslib_sprites.AnimationAll|khaoslib_sprites.SpriteAll|khaoslib_sprites.IconAll
--- @param sprites T The animation or sprite or icons (object) or a table of them.
--- @param tint data.Color A color table, e.g. {r=1, g=0.5, b=0, a=0.5}
--- @return T copy A copy of the given animation or sprite or icons (object) or a table of them with the tint applied.
--- @nodiscard
